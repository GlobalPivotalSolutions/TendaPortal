...
import au.csiro.cass.arch.security.ScanUtil;
...
public class ParseUtil {
  ...
  public ParseResult parse(Content content) throws ParseException {
    ...
    try { ScanUtil.preParsePrune( conf, content ) ; }
    catch( Exception e )
        { LOG.error( "Pruning " + content.getUrl() + " failed: " + e.getMessage() ) ; }
    ...
    for (int i = 0; i < parsers.length; i++) {
      ...
      if ( ( parseResult != null && parseResult.isSuccess() ) || i == parsers.length - 1 )
      {
        if ( parseResult.isSuccess() )
        {
          if ( i != 0 )
            LOG.info("Successfully parsed [" + content.getUrl() + "] with [" + parsers[i] + "]");
          try { ScanUtil.scan( conf, content, parseResult ) ; }
          catch( Exception e )
              { LOG.error( "Scanning " + content.getUrl() + " failed: " + e.getMessage() ) ; }
        }
        return parseResult ;
      }
    }
