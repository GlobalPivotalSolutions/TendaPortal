...
function linkClick( e ) //=====================================================
{
  ...
  u = addUrlArg( baseURL, "start=" + start ) ;
  retrieve( u, this ) ;
  scroll( 0, 0 ) ; 
  return false ;
}
...
function retrieve( url, box ) 
{
  ...
  request.requestFile = addUrlArg( url, "ar_format=resultsBody" ) + query ;
...
//==============================================================================
//  Adds an argument to URL when it is not known if the URL already has args
function addUrlArg( url, arg ) //===============================================
{  
  if ( url.indexOf('?') === -1) url = url + '?' + arg ;
   else url = url + '&' + arg ;
  return url ;
}
